...
largertitle <- element_text(size = 16)
...
plot2 <- ggplot(c6time)+
  ...
  ggtitle("Turbidity")+
  labs(y="Turbidity (NTU)", x="DateTime")
...
plot3 <- ggplot(c6time)+
  ...
  labs(y="Chlorophyll A (μg/l)", x="DateTime")
...
plot7 <- ggplot(c6time)+
  ...
  ggtitle("Station 1 CO2")+
  labs(y="C02 ppm", x="DateTime")
...
plot9 <- ggplot(c6time)+
  ...
  labs(y="Flux (μmol/m2/s)", x= "DateTime")
...
plot10 <- ggplot(c6time)+
  geom_polygon(data = disinjpoly2,aes(x=x,y=y),fill="#ff7722", alpha = .5)+
  ...
  labs(y="Discharge (l/s)", x="DateTime")
plot10 <- plot10+ theme(axis.text.y= largernumbers, axis.text.x = largernumbers, axis.title.x = largernumbers,
                        axis.title.y= largernumbers, title = largertitle)
ggplotly(plot10)
...
grid.newpage()
grid.draw(rbind(ggplotGrob(plot5),ggplotGrob(plot10), ggplotGrob(plot), ggplotGrob(plot2), ggplotGrob(plot3), ggplotGrob(plot9), ggplotGrob(plot7)))  
