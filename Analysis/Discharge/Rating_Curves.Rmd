...
```{r stn1Choice}
# Exponential Model:
#df$stn1_Q <- exp(predict(exponential.model,data.frame(x=df$lvl_436_m)))

# Quadratic Model
df$stn1_Q <- predict(quadratic.model,list(x=df$lvl_436_m, x2=df$lvl_436_m^2))
```

## Station 2 Rating Curve FOR REAL
Set up variables:
...
ggplot()+
  geom_point(aes(x,y))+
  geom_line(aes(x,exp.predict),col = 'red')+
  labs(x = "Level (m)", y = expression(paste("Discharge ", m^{3}/s^{-1})), title ="Station 2 Exponential Model ")
...
```{r stn2Choice}
# Exponential Model:
df$stn2_Q <- exp(predict(exponential.model,data.frame(x=df$lvl_436_m)))
...
```{r stn3Choice}
# Exponential Model:
df$stn3_Q <- exp(predict(exponential.model,data.frame(x=df$lvl_421_m)))

# Quadratic Model
#df$stn3_Q <- predict(quadratic.model,list(x=df$lvl_421_m, x2=df$lvl_421_m^2))
```
...
```{r stn4Choice}
# Exponential Model:
#df$stn4_Q <- exp(predict(exponential.model,data.frame(x=df$lvl_421_m)))

# Quadratic Model
df$stn4_Q <- predict(quadratic.model,list(x=df$lvl_421_m, x2=df$lvl_421_m^2))
```
