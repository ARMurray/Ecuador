...
plot13Ppm <- ggplot(filt13)+
  ...
  geom_point(aes(x = time, y = V3-V1,group = day, col=stn1_Q))+
  scale_color_gradient(name = "Discharge [L/s]", trans = "log",
                        breaks = my_breaks, labels = my_breaks)+
  labs(x = "Time",y= "&#916;CO<sub>2</sub> [ppm]", title = "DCO2 Change (station 3 minus station 1)")+
  ylim(-2650,150)+
  theme_bw()
...
plot34Ppm <- ggplot(filt34)+
  ...
  geom_vline(aes(xintercept = as.POSIXct("06:15", format = "%H:%M")))+
  geom_vline(aes(xintercept = as.POSIXct("18:21", format = "%H:%M")))+
  geom_point(aes(x = time, y = V4-V3,group = day, col=stn1_Q))+
  scale_color_gradient(name = "Discharge [L/s]", trans = "log",
                        breaks = my_breaks, labels = my_breaks)+
  labs(x = "Time",y="&Delta;CO<sub>2</sub>", title = "DCO2 Change (Station 2-1 / Station 4-3)")+
  ylim(-2650,150)+
  theme_bw()

#subplot(plot13Ppm,plot34Ppm, shareY = TRUE)

#Plotly replacement
plotly13Ppm <- filt13%>%
  plot_ly(x = ~time, y = ~V3-V1)%>%
  add_markers(color=~log10(stn1_Q))%>%
  layout(
    xaxis = list(
  ...
  tickangle = 325,
  tickfont = xaxisFont),
    title = "DCO2 Change (Station 2-1 / Station 4-3)",
         shapes = list(
               list(type = "rect",
                    fillcolor = "yellow", line = list(color = "yellow"), opacity = .2,
                    x0 = "2019-11-30 06:15", x1 = "2019-11-30 18:21", xref = "x",
...
plotly34Ppm <- filt34%>%
  plot_ly(x = ~time, y = ~V4-V3)%>%
  add_markers(color=~log10(stn3_Q))%>%
  layout(
    xaxis = list(
    range= c(as.numeric(todayStart)*1000,
             as.numeric(todayEnd)*1000),
...
subplot(plotly13Ppm,plotly34Ppm, shareY = TRUE, shareX = TRUE)
```
...
```{r DOchange, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10}
dayPoly <- data.frame(x = c(as.POSIXct("06:15", format = "%H:%M"),as.POSIXct("18:21", format = "%H:%M"),as.POSIXct("18:21", format = "%H:%M"),as.POSIXct("06:15", format = "%H:%M")),y=c(-1,-1,8,8))

plot12DO <- ggplot(filt13)+
  geom_polygon(data = dayPoly,aes(x=x,y=y),fill="#f2e198")+
  geom_vline(aes(xintercept = as.POSIXct("06:15", format = "%H:%M")))+
  geom_vline(aes(xintercept = as.POSIXct("18:21", format = "%H:%M")))+
  geom_point(aes(x = time, y = DO2_mg.L-DO1_mg.L,group = day, col=tempC_436),lwd=1)+
  scale_color_gradient(name = "Discharge (L/s)", trans = "log",
                        breaks = my_breaks, labels = my_breaks)+
...
plot24DO <- ggplot(filt34)+
  ...
  geom_vline(aes(xintercept = as.POSIXct("06:15", format = "%H:%M")))+
  geom_vline(aes(xintercept = as.POSIXct("18:21", format = "%H:%M")))+
  geom_point(aes(x = time, y = DO2_mg.L,group = day, col=tempC_436),lwd=1)+
  scale_color_gradient(name = "Discharge (L/s)", trans = "log",
                        breaks = my_breaks, labels = my_breaks)+
  labs(x = "Time",y="DO (mg/L)", title = "DO Change (station 2-1 / Station 4-2)")+
  theme_bw()

subplot(plot12DO,plot24DO,shareY = TRUE)
...
```{r CO2ChangeTime, echo=FALSE, warning=FALSE}
...
cChange13 <- ggplot(filt13)+
  geom_point(aes(x=DateTime, y= uMols_13, col = noonDif))+
  scale_color_gradient2(low = "black",mid = "#f2db79", high = "black",name = "Time")+
  theme_bw()+
  ylim(-10,60)

cChange34 <- ggplot(filt34)+
  geom_point(aes(x=DateTime, y= uMols_34, col = noonDif))+
  scale_color_gradient2(low = "black",mid = "#f2db79", high = "black",name = "Time")+
  theme_bw()+
  ylim(-10,60)
...
## ggplot
cChguMols13 <- ggplot(filt13)+
  geom_polygon(data = dayPoly,aes(x=x,y=y),fill="#f2e198")+
  geom_vline(aes(xintercept = as.POSIXct("06:15", format = "%H:%M")))+
  geom_vline(aes(xintercept = as.POSIXct("18:21", format = "%H:%M")))+
  geom_point(aes(x = time, y = uMols_13,group = day, col=stn1_Q))+
  scale_color_gradient(name = "Discharge (L/s)", trans = "log",
                        breaks = my_breaks, labels = my_breaks)+
  labs(x = "Time",y="Evasion", title = "CO2 Change 1 -> 3 (uMols)")+
  theme_bw()

cChangeuMols34 <- ggplot(filt34)+
  geom_polygon(data = dayPoly,aes(x=x,y=y),fill="#f2e198")+
  geom_vline(aes(xintercept = as.POSIXct("06:15", format = "%H:%M")))+
  geom_vline(aes(xintercept = as.POSIXct("18:21", format = "%H:%M")))+
  geom_point(aes(x = time, y = uMols_34,group = day, col=stn1_Q))+
  scale_color_gradient(name = "Discharge (L/s)", trans = "log",
                        breaks = my_breaks, labels = my_breaks)+
  labs(x = "Time",y="Evasion", title = "CO2 Change 3 -> 4 (uMols)")+
  theme_bw()

subplot(cChguMols13,cChangeuMols34, shareY = TRUE)

## Plotly
cChguMols13 <- filt13%>%
  group_by(day)%>%
  plot_ly(x = ~time, y = ~uMols_13)%>%
  add_markers(color = ~stn1_Q)
