...
```{r}
ggplot(df)+
  geom_histogram(aes(x = flux_12),binwidth = 10)+
  xlim(-20,250)
```
...
ggplot()+
  geom_bar(data = pptDaily,stat = "identity", aes(x = DateTime, y = ppt_mm))+
  geom_point(data = nonInjections,aes(x = DateTime, y = flux_12/50))+
  labs(x = "Date", y="ppt", title = "Flux: Station 1 -> 2")

ggplot()+
  geom_bar(data = pptDaily,stat = "identity", aes(x = DateTime, y = ppt_mm))+
  geom_point(data = nonInjections,aes(x = DateTime, y = flux_14/50))+
