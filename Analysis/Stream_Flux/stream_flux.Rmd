...
pred1 <- read.csv(here::here("Analysis/Stream_Metabolism/ModelOutputs/DO_1_Predictions.csv"))%>%
  select(date,GPP,ER)%>%
  mutate(Station = "1")
pred2 <- read.csv(here::here("Analysis/Stream_Metabolism/ModelOutputs/DO_2_Predictions.csv"))%>%
  select(date,GPP,ER)%>%
  mutate(Station = "2")
pred4 <- read.csv(here::here("Analysis/Stream_Metabolism/ModelOutputs/DO_4_Predictions.csv"))%>%
  select(date,GPP,ER)%>%
  mutate(Station = "4")
...
adjustEvasion <- metabAvg%>%
  ...
  mutate(AdjEvasion13 = DailyChange13+GPP_CO2uMol12+ER_CO2uMol12,
...
up <- adjustEvasion%>%
  select(date,AdjEvasion13,ER_CO2uMol12,GPP_CO2uMol12)%>%
  rename("Evasion" = "AdjEvasion13", "Respiration"="ER_CO2uMol12", "Photosynthesis" = "GPP_CO2uMol12")%>%
  gather(Var,Val,-date)
...
```{r difCheck, echo=FALSE}
# Check Percent Difference
difCheck <- adjustEvasion%>%
  mutate(pctDifUp = 100*((DailyChange13-AdjEvasion13) / AdjEvasion13),
         pctDifDown = 100*((DailyChange34-AdjEvasion34) / AdjEvasion34))%>%
  select(date,DailyChange13,AdjEvasion13,pctDifUp,DailyChange34,AdjEvasion34,pctDifDown)%>%
  drop_na()

colnames(difCheck) <- c("Date","Change_1-3","Evasion_1-3","Pct_Dif_1-3","Change_3-4","Evasion_3-4","Pct_Dif_3-4")  
kableExtra::kable(difCheck)

 ## Plot it
adjustEvasion%>%
  plot_ly(x=~DailyChange13,y=~AdjEvasion13)%>%
  add_markers()%>%
  add_segments(x = 0,y=0,xend = 400000,yend=400000)
...
# Fit regression line
fit <- lm(vFlux ~ eosFlux, data = fluxCompare, na.rm=TRUE)
