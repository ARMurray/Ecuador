...
#read in file
dat <- read.csv(here::here("Analysis/Stream_Metabolism/EC_IRU1_2020-03-10_XX.csv"))
#rename columns
names(dat)[names(dat) == "Discharge_m3L"] <- "discharge"
...
#dat$depth <- dat$Level_m*1.6396 - 0.1748     this does not look correct
...
#use when pool_K600 is 'none'. Use K600 none as last resort
#bayes_specs_new$K600_daily_meanlog = log(K)
#bayes_specs_new$K600_daily_sdlog = 0.001

dat_metab = metab(bayes_specs_new, data=dat)
dat_fit = get_fit(dat_metab)
n_KQ_nodes = 7 # should be 7 <= x <= 17; increase with discharge range. (Kriddie, keep yours at 7 probably)
q_range = range(daily_mean_discharge, na.rm=TRUE)
modspecs$K600_lnQ_nodes_centers = seq(from=q_range[1],
                                      to=q_range[2], length.out=n_KQ_nodes)
