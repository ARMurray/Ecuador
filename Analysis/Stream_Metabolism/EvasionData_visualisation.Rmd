...
evasion.df <- evasion[,c("DateTime", "Inj","V1","V2","V3","Flux_1","ppt_mm", "DO2_mg.L")]
...
evasion.df$change <- evasion.df$V1 - evasion.df$V3
...
evasion.df <- evasion.df[!with(evasion.df,is.na(Flux_1)& is.na(change)),]
...
plot(df$Flux_1, df$change)
abline(fit <- lm(df$change ~ df$Flux_1), col="red")
legend("topright", bty="n", legend=paste("R2 is", 
         format(summary(fit)$adj.r.squared, digits=4)))
